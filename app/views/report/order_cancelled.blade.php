    ...
    <table id="table"
            ...
            <thead>
             <tr>
                ...
                <th data-field="productIdx">ລະຫັດອໍເດີ້</th>
                <th data-field="product_name">ລາຍລະອຽດ</th>
...
$(document).ready(function(){
	...
	var $table = $('#table');
		...
		$table.bootstrapTable('refresh', {
			url : "{{ URL::to('report/order/cancelled/json?date_from="+$date_from.val()+"&date_to="+$date_to.val()+"&order=asc&limit=10&offset=0') }}"
		});
	...
	$btn_date.click(function(e){
		e.preventDefault();
		$table.bootstrapTable('refresh', {
			url : "{{ URL::to('report/order/cancelled/json?search=byDate&date_from="+$date_from.val()+"&date_to="+$date_to.val()+"&order=asc&limit=10&offset=0') }}"
		});
	});
